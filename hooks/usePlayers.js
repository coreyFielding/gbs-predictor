  ...
  const assignWeightsAndPositions = () => {
    variables?.forEach(({ Variable }, variableIndex) => {
      Variable.data.map((item, index) => {
        ...
        playerList[playerIndex].weights = weights ? [...weights] : [];

        playerList[playerIndex].weights[variableIndex] = {
          ...playerList[playerIndex].weights[variableIndex],
          position: parseFloat(item.Position), // <-- Here is where the position becomes incorrect
        };

        newPlayerList.push({
          ...playerList[playerIndex],
          weights: [playerList[playerIndex].weights[variableIndex]],
        });
      });
    });
  };
