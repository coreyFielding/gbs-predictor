...
const queryAllGroups = async (id = 1) => {
  try {
    ...
    const groupsEndpoint = `http://localhost:1337/api/variable-filters/${id}?populate=*`
    ...
    const variablesEndpoint = `http://localhost:1337/api/variable-filters/1?populate[0]=${groups.join(
      ","
